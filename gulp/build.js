...
gulp.task('clean', function () {
	return $.del([path.join(conf.paths.dist, '/'), path.join(conf.paths.tmp, '/'), './admin.zip']);
});

gulp.task('build', ['html', 'fonts', 'other'], function () {
    // var buildSrc = gulp.src(path.join(conf.paths.dist, '/**/*'), {read: true});
    /*var buildSrc = gulp.src(path.join(conf.paths.dist, '/!**!/!*'), {base: path.join(conf.paths.dist, '/')});
    buildSrc
		.pipe($.zip('admin.zip'))
     .pipe(gulp.dest('./'));*/
});
